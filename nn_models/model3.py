...
# weight + activity regularization
...
for i in range(k):
    model = Sequential()
    model.add(Dense(500, input_dim=46, init='uniform', W_regularizer=l2(0.01), activity_regularizer=activity_l2(0.01)))
    model.add(Activation('tanh'))
    model.add(Dropout(0.5))
    model.add(Dense(400, init='uniform', W_regularizer=l2(0.01), activity_regularizer=activity_l2(0.01)))
    model.add(Activation('tanh'))
    model.add(Dropout(0.5))
    model.add(Dense(39, init='uniform', W_regularizer=l2(0.01), activity_regularizer=activity_l2(0.01)))
...
avg_loss, avg_accs, allypred = run_model(Xtrain_d, ytrain, indtrain1, indtrain2, list_of_nn_models, nb_epoch,
                                         model_name)
